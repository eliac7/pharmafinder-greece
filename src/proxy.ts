...
import { PUBLIC_ID_PATTERN } from '@/entities/pharmacy/lib/public-url'
...
function routeReferenceKind(segment: string) {
  ...
  if (PUBLIC_ID_PATTERN.test(segment)) return 'public_id'
  if (segment.length > 24 && segment.slice(-24, -22) === '--') return 'canonical'
  return 'malformed'
}
...
export async function proxy(request: NextRequest) {
  const pharmacyMatch = request.nextUrl.pathname.match(/^\/farmakeia\/([^/]+)$/)
  if (pharmacyMatch) {
