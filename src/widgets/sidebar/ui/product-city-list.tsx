...
export function ProductCityList({ citySlug, time }: { citySlug: string; time: DutyTime }) {
  ...
  const loadMore = async () => {
    const cursor = nextCursor ?? query.data?.next_cursor;
  ...
  return <div className="flex min-h-0 flex-1 flex-col"><div className="px-1 pb-2 text-xs font-semibold text-muted-foreground">ΦΑΡΜΑΚΕΙΑ ({currentItems.length}{query.data?.has_more ? "+" : ""}){coverage}</div><ScrollArea className="flex-1"><div className="flex flex-col gap-3 pr-3 pb-2">{currentItems.map((item) => <ActionPharmacyCard key={item.handle} item={item} timeFilter={time} />)}{(nextCursor || query.data?.has_more) && <Button variant="outline" size="sm" onClick={() => void loadMore()} disabled={loadingMore}>{loadingMore ? <Loader2 className="size-4 animate-spin" /> : "Φόρτωση περισσότερων"}</Button>}</div></ScrollArea></div>;
}
