...
function ActionLayer({
  ...
  response: MapActionResponse;
  timeFilter: DutyTime;
  onDrill: (next: MapActionResponse) => void;
}) {
  const { map } = useMap();
        ...
        ? response.clusters.map((cluster) => (
            ...
            >
              <MarkerContent>
                <button
                  type="button"
                  onClick={() => void drill(cluster.handle)}
                  className="flex size-10 items-center justify-center rounded-full border-2 border-primary bg-primary text-xs font-bold text-primary-foreground shadow-lg"
                >
                  {cluster.count}
                </button>
              </MarkerContent>
              <MarkerTooltip>{cluster.count} φαρμακεία</MarkerTooltip>
            </MapMarker>
          ))
            ...
            >
              <MarkerContent>
                <button
                  type="button"
                  onClick={() =>
                    void open(marker.handle, [
                      marker.longitude,
                      marker.latitude,
                    ])
                  }
                  disabled={isMarkerLoading(marker.handle)}
                  aria-busy={isMarkerLoading(marker.handle)}
                  className="cursor-pointer border-0 bg-transparent p-0 disabled:cursor-wait"
                >
                  <ProductActionMarkerContent
                    publicId={marker.public_id}
                    status={
                      marker.duty_summary
                        ? getDutySummaryStatus(marker.duty_summary, timeFilter)
                            .status
                        : timeFilter === "now"
                          ? "open"
                          : "scheduled"
                    }
                    isLoading={isMarkerLoading(marker.handle)}
                  />
...
export function ProductActionViewport() {
  ...
  const [pendingBounds, setPendingBounds] = useState<ViewportBounds | null>(
    null,
  );
  const [committedBounds, setCommittedBounds] = useState<ViewportBounds | null>(
    null,
  );
  const [drilledResponse, setDrilledResponse] =
    useState<MapActionResponse | null>(null);
  const movementRef = useRef(false);
  const query = useQuery({
    queryKey: ["product-map", committedBounds, time],
    queryFn: async () => {
      const requestedBounds = committedBounds!;
      const result = await queryMapAction(
        requestedBounds,
        Math.floor(map?.getZoom() ?? 12),
        time,
      );
      setPendingBounds((current) =>
        current && sameBounds(current, requestedBounds) ? null : current,
      );
      return result;
    },
  ...
  useEffect(() => {
    ...
    const start = (event: MoveEvent) => {
      movementRef.current = Boolean(event.originalEvent);
    };
    const end = () => {
  ...
  const searchArea = () => {
    ...
    if (
      query.error &&
      committedBounds &&
      sameBounds(pendingBounds, committedBounds)
    ) {
      void query.refetch();
      return;
    }
  ...
  return (
    <>
      ...
      {(pendingBounds || query.isFetching) && (
        <div className="absolute top-5 left-1/2 z-20 -translate-x-1/2">
          <Button
            ...
            className="rounded-full border border-white/10 bg-[rgba(20,20,30,0.82)] px-4 text-white shadow-lg backdrop-blur-sm"
          >
            {query.isFetching ? (
      ...
      {response?.duty_coverage.status === "partial" && (
        <div className="absolute bottom-5 left-1/2 z-10 -translate-x-1/2 rounded-full bg-amber-100 px-3 py-1 text-xs text-amber-800 shadow">
          Μερική κάλυψη εφημεριών
        </div>
      )}
    </>
  );
